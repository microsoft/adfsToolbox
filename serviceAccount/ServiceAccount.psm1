...
function Set-AdfsServiceAccountRule
{
    param(
    ...
    $Account = $ServiceAccount.Split('\') #Input should have been given in the format domain\user
    $User = $Account[1]
    $Lookup = Get-ADUser -Filter {Name -eq $User} 
    if($Lookup -eq $null)
    {
        Write-Error "The specified account does not exist"
    }
    ...
    if((IsWID -ConnectionString $connString) -eq $true)
    {
        ...
        {
            Write-Warning("No list of secondary servers was provided. You must ensure a a sync has occurred on all machines before proceeding to change the service account.")
        }
...
function Set-AdfsServiceAccount 
{
...
Export-ModuleMember -Function Set-AdfsServiceAccountRule
Export-ModuleMember -Function Set-AdfsServiceAccount